...
<%
	...
	int maxFileSize = 1024 * 1024 * 100; // 100Mbyte 파일사이즈는 byte로 표시 
	...
	// input type = text
	String itemName = mreq.getParameter("itemName"); // MultipartRequest.getParameter, 파일은 upload에 저장

	// input type = file 바이너리파일은 마임타임 형태의 파일로 변환되어 upload폴더에 자동으로 저장
	String contentType = mreq.getContentType("itemImg");
	String originalFileName = mreq.getOriginalFileName("itemImg"); // 원본파일이름
	String fileSystemName = mreq.getFilesystemName("itemImg"); // 저장된 파일 이름(DefaultFileRenamePolicy에 의해서...)
